    ...
    class TripleWeapon : Weapon
    {
        ...
        public override void Fire(GameTime gameTime)
        {
            ...
            if (gameTime.TotalGameTime - _previousLaserSpawnTime > _laserSpawnTime)
            {
                ...
                _game.AddLaser(ghaMovement.create(_player.Position, -10f, -30.0f * (float)Math.PI / 180.0f, -30.0f * (float)Math.PI / 180.0f));
                _game.AddLaser(ghaMovement.create(_player.Position, 0f, 0.0f, 0.0f));
                _game.AddLaser(ghaMovement.create(_player.Position, 10f, 30.0f * (float)Math.PI / 180.0f, 30.0f * (float)Math.PI / 180.0f));
            }
